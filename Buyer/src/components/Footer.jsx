...
const NavFooterLink = ({ href, text }) => {
  return (
    <li>
      <a href={href} className="nav_a">{text}</a>
    </li>
  );
};

const NavFooterColumn = ({ heading, links }) => {
  return (
    <div className="">
      <h2 className="text-xl font-bold mb-4">{heading}</h2>
      <ul className=''>
        {links.map((link, index) => (
          <NavFooterLink key={index} href={link.href} text={link.text} />
        ))}
      </ul>
    </div>
  );
...
function Footer() {
  ...
  return (
    <>
    <div className="container mx-auto px-4 py-8" role="presentation">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        {columns.map((column, index) => (
          <NavFooterColumn key={index} heading={column.heading} links={column.links} />
        ))}
      </div>
    </div>
